...
Z = as.matrix(df1[controls_exog])
X = as.matrix(df1[var_hte])
D=df1$B_treat
...
D.boost = cvboost(Z,D,objective = "binary:logistic",nthread = 4, k_folds = 5,ntrees_max = 500)
...
```{r}
mean(tau.hat.boost[which(df1$B_Sgirl==1)])
mean(tau.hat.boost[which(df1$B_Sgirl==0)])
```
