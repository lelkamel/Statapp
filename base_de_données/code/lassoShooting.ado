...
// Partial out controls, constant
	foreach x of local newvarlist {
		local varname = substr("`x'", 1, 30) //added from jake to deal with long variable names
		quietly regress `x' `controls' if `touse'
		tempvar r`varname'
		quietly predict `r`varname'' if e(sample), r
		local rvarlist : list rvarlist | r`varname'
	}
